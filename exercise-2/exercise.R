...
length(distinct(vehicles))

# Filter the data set for vehicles manufactured in 1997
cars_1997 <- filter(vehicles, year == "1997")

# Arrange the 1997 cars by highway (`hwy`) gas milage
sorted_cars <- arrange(cars_1997, hwy)

# Mutate the 1997 cars data frame to add a column `average` that has the average
# gas milage (between city and highway mpg) for each car
average_cars <- mutate(cars_1997, average = (hwy + cty ) / 2)

# Filter the whole vehicles data set for 2-Wheel Drive vehicles that get more
# than 20 miles/gallon in the city. 
# Save this new data frame in a variable.
more_20 <- filter(vehicles, drive == '2 Wheel Drive', cty > 20)

# Of the above vehicles, what is the vehicle ID of the vehicle with the worst 
# hwy mpg?
# Hint: filter for the worst vehicle, then select its ID.
filtered <- filter(more_20, hwy == min(hwy))
